...
CREATE TABLE public.pop_uploads (
    ...
    CONSTRAINT valid_payment_fields CHECK (
        (upload_type = 'proof_of_payment' AND payment_amount IS NOT NULL AND payment_date IS NOT NULL) OR
        (upload_type = 'picture_of_progress' AND subject IS NOT NULL)
    ),
...
CREATE OR REPLACE FUNCTION get_pop_upload_stats(target_student_id UUID DEFAULT NULL, target_preschool_id UUID DEFAULT NULL)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
