    ...
    student_id IN (
        ...
        AND organization_id IN (
            SELECT organization_id FROM profiles WHERE id = (
                SELECT student_id FROM assignment_access WHERE id = assignment_access.id
            )
        )
    )
);
...
USING (
    EXISTS (
        ...
        WHERE a.id = assignment_access.assignment_id
...
USING (
    EXISTS (
        ...
        JOIN profiles p ON p.id = auth.uid()
        WHERE c.id = course_grades.course_id
        AND (c.instructor_id = auth.uid() OR p.role = 'admin')
        AND p.organization_id = c.organization_id
    )
);
...
USING (
    ...
    EXISTS (
        ...
        WHERE spr.student_id = course_grades.student_id
...
USING (
    EXISTS (
        SELECT 1 FROM courses c
        JOIN profiles p ON p.id = auth.uid()
        WHERE c.id = gradebook_entries.course_id
        AND (c.instructor_id = auth.uid() OR p.role = 'admin')
        AND p.organization_id = c.organization_id
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM student_parent_relationships spr
        JOIN profiles parent ON parent.id = auth.uid()
        WHERE spr.student_id = gradebook_entries.student_id
        AND spr.parent_id = auth.uid()
        AND parent.role = 'parent'
    )
);
...
USING (
    EXISTS (
        ...
        AND (
            ...
            organization_id IS NULL -- System-wide tasks
        )
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM student_parent_relationships spr
        JOIN profiles parent ON parent.id = auth.uid()
        WHERE spr.student_id = student_progress.student_id
        AND spr.parent_id = auth.uid()
        AND parent.role = 'parent'
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM profiles p
        JOIN courses c ON c.id = student_progress.course_id
        WHERE p.id = auth.uid()
        AND p.role IN ('admin', 'principal')
        AND p.organization_id = c.organization_id
    )
