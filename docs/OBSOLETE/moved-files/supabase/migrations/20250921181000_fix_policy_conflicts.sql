...
USING (
    EXISTS (
        SELECT 1 FROM profiles
        WHERE id = auth.uid()
        AND organization_id = petty_cash_accounts.preschool_id
        AND role IN ('admin', 'principal')
    )
);
...
USING (
    EXISTS (
        ...
        AND (
            ...
            (activity_logs.organization_id IS NOT NULL AND organization_id = activity_logs.organization_id)
            OR
            ...
            (activity_logs.organization_id IS NULL AND activity_logs.school_id IS NOT NULL 
             AND organization_id = activity_logs.school_id)
        )
    )
);
...
WITH CHECK (
    EXISTS (
        SELECT 1 FROM profiles
        WHERE id = auth.uid()
        AND organization_id IN (activity_logs.organization_id, activity_logs.school_id)
    )
);
