...
CREATE INDEX IF NOT EXISTS idx_petty_cash_accounts_active ON public.petty_cash_accounts(is_active) WHERE is_active = true;
...
COMMENT ON FUNCTION public.get_petty_cash_summary(UUID, DATE, DATE) IS 'Returns summary statistics for petty cash accounts and transactions within date range';