...
-- Create a simple superladmin policy without self-reference
...
-- Insert a superladmin user manually (replace with actual auth user ID)
...
BEGIN
    ...
    IF current_user_id IS NOT NULL THEN
        ...
        -- Insert/update the current user as superladmin
        INSERT INTO user_profiles (
            ...
            current_user_id,
            COALESCE(user_email, 'superladmin@edudashpro.org.za'),
            'Super Administrator',
            'super_admin',
            true
        ...
        RAISE NOTICE 'Updated user % as superladmin', user_email;
    ELSE
        RAISE NOTICE 'No authenticated user found - will create default superladmin';
        
        -- Create a default superladmin entry (this requires knowing the auth user ID)
        ...
        RAISE NOTICE 'Created placeholder superladmin - update with real auth user ID';
    END IF;
END $$;
